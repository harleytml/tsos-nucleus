...
bool FAT16_quark::isfilenamevalid(char *name)
{
    ...
    while (name[pos])
    {
        ...
        isvalid = ('A' <= c && c <= 'Z') || ('0' <= c && c <= '9') || c == '!' || c == '#' || c == '$' || c == '%' ||
                  c == '&' || c == '\'' || c == '(' || c == ')' || c == '-' || c == '@' || c == '^' || c == '_' ||
                  c == '`' || c == '{' || c == '}' || c == '~';
...
bool FAT16_quark::deleteentryinroot(char *name, bool is_file)
{
    ...
    if ((is_file && (entry.attribute & static_cast<uint8_t>(FILE_ATTRIBUTE::SUBDIR))) ||
        (!is_file && !(entry.attribute & static_cast<uint8_t>(FILE_ATTRIBUTE::SUBDIR))))
    {
        return -1;
    }
...
void FAT16_quark::markrootentryasavailable(uint16_t entry_index)
{
    ...
    movetorootdirectoryregion(entry_index);
    tsos->disk.setbytes(seekpoint, sizeof(entry), reinterpret_cast<uint8_t *>(&entry));
}

...
void *Memory::allocatememory(uint32_t len)
{
    ...
    // A counter to make sure the new allocated space is not conflicting with the
    // other entries
        ...
        for (uint32_t x = 0; x < MEMORY_BLOCK_COUNT; x++)
        {
            if (&memory_table[x] != possible_entry)
            {
                ...
                if (memory_table->is_active)
                {
                    ...
                    if ((possible_address <= memory_table[x].memory_start + memory_table[x].block_length) &&
                        (memory_table[x].memory_start <= possible_address + len))
                    {

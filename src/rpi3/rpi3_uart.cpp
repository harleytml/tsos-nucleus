...
void RPI3_UART_quark::reset(void)
{
	...
	mmio_write((uint32_t)registers::GPPUD, 0x00000000);
	delay(150);
 
	// Disable pull up/down for pin 14,15 & delay for 150 cycles.
	mmio_write((uint32_t)registers::GPPUDCLK0, (1 << 14) | (1 << 15));
	delay(150);
 
	// Write 0 to GPPUDCLK0 to make it take effect.
	mmio_write((uint32_t)registers::GPPUDCLK0, 0x00000000);
 
	// Clear pending interrupts.
	mmio_write((uint32_t)registers::UART0_ICR, 0x7FF);
	...
	while ( mmio_read((uint32_t)registers::MBOX_STATUS) & 0x80000000 ) { }
	// send our message to property channel and wait for the response
	mmio_write((uint32_t)registers::MBOX_WRITE, r);
	while ( (mmio_read((uint32_t)registers::MBOX_STATUS) & 0x40000000) || mmio_read((uint32_t)registers::MBOX_READ) != r ) { }

...
void DMG_SCREEN_driver::reset(void)
{
  ...
  *screen_control = 0x81;
  ...
  // Set the background scroll of the X axis to 0
  screen_control = (uint8_t *)0xff43;
  *screen_control = 0x00;

  // Set the background scroll of the Y axis to 0
...
/* So, this function is intereseting. The DMG is pretty much set up so that you can write plain text to a buffer and text appears on the screen.
   ...
   So the screen driver must only be able to jump within the range of the visible screen.
...
uint16_t DMG_SCREEN_driver::getscreenwidth(void)
{
  // The screen is 160*144, with 8*8 characters
  return 20;
}

uint16_t DMG_SCREEN_driver::getscreenheight(void)
{
  // The screen is 160*144, with 8*8 characters
  return 18;
}
